...
namespace pcl
{
    ...
    class Kinect2Grabber : public pcl::Grabber
    {
        public:
            ...
            typedef void ( signal_Kinect2_dataa) (cv::Mat& );
        ...
        protected:
            ...
            boost::signals2::signal<signal_Kinect2_dataa>* signal_dataa;
            ...
            cv::Mat convertData(RGBQUAD* colorBuffer, UINT16* depthBuffer);
        ...
        , signal_dataa( nullptr )
    {
        ...
        signal_dataa = createSignal<signal_Kinect2_dataa>();
    }
    ...
    pcl::Kinect2Grabber::~Kinect2Grabber() throw()
    {
		...
		disconnect_all_slots<signal_Kinect2_dataa>();
    ...
    void pcl::Kinect2Grabber::threadFunction()
    {
        while( !quit ){
            ...
            if ( signal_dataa->num_slots() > 0 ){
                signal_dataa->operator()( convertData( &colorBuffer[0], &depthBuffer[0] ) );
            }
        }
    }
    ...
    cv::Mat pcl::Kinect2Grabber::convertData( RGBQUAD* colorBuffer, UINT16* depthBuffer )
    {
		pcl::dataa* retr;
		pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZRGBA>());
		cv::Mat rgb, depth;
		rgb = cv::Mat(colorHeight, colorWidth, CV_8UC4);
		cloud->width = static_cast<uint32_t>(depthWidth);
		cloud->height = static_cast<uint32_t>(depthHeight);
		cloud->is_dense = false;
		...
		for (int i = 0; i < colorHeight; i++) {
			for (int j = 0; j < colorWidth; j++) {
				RGBQUAD color = colorBuffer[i * colorWidth + j];
				rgb.at<cv::Vec4b>(cv::Point(j, i))[0] = color.rgbBlue;
				rgb.at<cv::Vec4b>(cv::Point(j, i))[1] = color.rgbGreen;
				rgb.at<cv::Vec4b>(cv::Point(j, i))[2] = color.rgbRed;
				rgb.at<cv::Vec4b>(cv::Point(j, i))[3] = color.rgbReserved;
